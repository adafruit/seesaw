...
#define CONFIG_GPIO_MASK ((0xFFFFFFFFul) ^ (( (CONFIG_USB << PIN_USB_DM) | (CONFIG_USB << PIN_USB_DP) | (1ul << PIN_ADDR_0) | (1ul << PIN_ADDR_1) | (1ul << PIN_ACTIVITY_LED) ) \
		...
		| ((uint32_t)CONFIG_SPI_SLAVE << CONFIG_SPI_SLAVE_PIN_MISO) \
		| ((uint32_t)CONFIG_SPI_SLAVE << CONFIG_SPI_SLAVE_PIN_SCK) \
		| ((uint32_t)CONFIG_SPI_SLAVE << CONFIG_SPI_SLAVE_PIN_SS) \
...
#define CONFIG_NUM_AO (1 + CONFIG_ADC + CONFIG_DAC + CONFIG_TIMER + CONFIG_INTERRUPT + CONFIG_SERCOM0 + CONFIG_SERCOM1 + CONFIG_SERCOM2 + CONFIG_SERCOM3 + \
		CONFIG_SERCOM4 + CONFIG_SERCOM5 + CONFIG_I2C_SLAVE + CONFIG_SPI_SLAVE + CONFIG_DAP + CONFIG_NEOPIXEL + CONFIG_USB)
